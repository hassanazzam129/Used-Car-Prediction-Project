...
car_brand = st.radio('Select Brand', brands, horizontal= True)
car_models = tuple(df[df['Brand'] == car_brand]['Model'].unique())
model = st.selectbox('Select Model', car_models, key= 'model_box')
...
# page 
cont = st.container()
body_col, trans_col = cont.columns(2)
with body_col:
    body = st.radio(
                "Select body type",
                body,
...
engine = st.selectbox('Select engine size', engine_sizes)

fuel = st.selectbox('Select fuel type', fuel_type)
...
pred_sample = {'Brand': car_brand, 'Model': model, 'Body': body, 'Transmission': trans, 'Engine': engine,
